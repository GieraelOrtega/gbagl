...
function validateBucketCompletion(body) {
  if (body.completed === '0') return null;
  if (body.completed !== '1') throw new Error('Completion status is invalid');
  return isoDate(body.completed_at, 'Completion date');
}
...
function validateJournal(body) {
  return {
    milestoneId: optionalId(body.milestone_id, 'Timeline milestone'),
    title: text(body.title, 'Title', 150),
    body: text(body.body, 'Journal body', 20000, { required: false }),
